...
P.config = function()
  ...
  H.dmypy = {
    ...
    name = "dmypy",
    stdin = H.mypy.stdin,
    ignore_exitcode = H.mypy.ignore_exitcode,
    append_fname = false,
    args = {
      "run",
      "--timeout", "600",
    ...
    parser = H.mypy.parser,
  }
  lint.linters.dmypy = H.dmypy
  ...
  H.python_linters = function()
    ...
    if command.executable(H.dmypy.cmd) then
      table.insert(linters, H.dmypy.cmd)
    elseif command.executable(H.mypy.cmd) then
